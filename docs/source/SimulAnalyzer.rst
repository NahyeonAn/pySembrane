...
A set of ordinary differential equations in Eqs. (\ref{eq:flux})-(\ref{eq:deltaP_Feedtube}) and pressure drop equations are solved using the following boundary conditions. For the feed stream, inlet conditions (at z = 0) are given:
